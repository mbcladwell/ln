...
public class HitListViewer extends JDialog implements java.awt.event.ActionListener {
  ...
  private static final long serialVersionUID = 1L;
    ...
    public HitListViewer(Session _s, int _hit_list_id) {
	session = _s;
    ...
    parent_pane = new JPanel(new BorderLayout());
        ...
        hits_table = session.getDatabaseRetriever().getSamplesForHitList(hit_list_id);
    ...
    GridLayout buttonLayout = new GridLayout(1,4,5,5);
      ...
      all_hit_lists_in_project = new JComboBox<ComboItem>(session.getDatabaseRetriever().getHitListsForProject(current_project_id));
    ...
    */
    ...
    //fails as long
    counts_table = session.getDatabaseRetriever().getHitCountPerPlateSet((session.getProjectID()), hit_list_id);
   ...
   hlButtons.add(closeButton);
	...
	JTable new_hits_table = session.getDatabaseRetriever().getSamplesForHitList(selected_hit_list_id);
	TableModel new_model = new_hits_table.getModel();
	hits_table.setModel(new_model); 
	
 
	JTable new_counts_table = session.getDatabaseRetriever().getHitCountPerPlateSet(session.getProjectID(), selected_hit_list_id);
