...
public class DialogRearrayHitList extends JDialog {
  ...
  // final EntityManager em;
    ...
    formatList.addActionListener(
        (new ActionListener() {
	      ...
	      layoutNames = session.getDatabaseRetriever().
    ...
    ComboItem[] layoutTypes = session.getDatabaseRetriever().
	getSourcePlateLayoutNames((int)formatList.getSelectedItem(),									                       ((ComboItem)typeList.getSelectedItem()).getKey());
  ...
  private void refreshPlateNumberLabel() {
      unknowns_per_dest_plate = session.getDatabaseRetriever().getUnknownCountForLayoutID(((ComboItem)layoutList.getSelectedItem()).getKey());
