...
public int new_plate_format_id;
    ...
    public int number_of_sample_replicates = 1;
    public int number_of_target_replicates = 4;
  ...
  case "1536":
    ...
    ComboItem old_plate_layout_name = session.getDatabaseRetriever().getPlateLayoutNameAndID(old_plate_layout_name_id);
...
target_reps_list_model =  new DefaultComboBoxModel<ComboItem>( target_reps );
    ...
    dest_layout = session.getDatabaseRetriever().getLayoutDestinationsForSourceID(old_plate_layout_name_id,1,4);
    //layout_names_list_model = new DefaultComboBoxModel<ComboItem>( layoutTypes );

    
    destLayoutLabel = new JLabel(dest_layout[0].toString());
    ...
    target_layout_names = session.getDatabaseRetriever().getTargetLayoutNamesForProject( session.getProjectID(), 4);
   ...
   label = new JLabel("(Optional)", SwingConstants.LEFT);
	...
	dbm
	 ...
	 number_of_sample_replicates = ((ComboItem)sampleRepsList.getSelectedItem()).getKey();
	 ...
	 case 1:
	      ...
	      //	ComboItem[] layoutTypes = session.getDatabaseRetriever().getLayoutDestinationsForSourceID(old_plate_layout_name_id,number_of_sample_replicates,((ComboItem)targetRepsList.getSelectedItem()).getKey());
	      dest_layout = session.getDatabaseRetriever().getLayoutDestinationsForSourceID(old_plate_layout_name_id,number_of_sample_replicates,((ComboItem)targetRepsList.getSelectedItem()).getKey());
		//layout_names_list_model = new DefaultComboBoxModel<ComboItem>( layoutTypes  );
		//layoutList.setModel(layout_names_list_model);
		//layoutList.setSelectedIndex(0);
	 ...
	 case 2:
	    targetRepsList.setModel(new DefaultComboBoxModel<ComboItem>( new ComboItem[]{ new ComboItem(2,"2"), new ComboItem(4,"4")}  ));
	      ...
	      targetRepsList.setEnabled(true);
	       dest_layout = session.getDatabaseRetriever().getLayoutDestinationsForSourceID(old_plate_layout_name_id,number_of_sample_replicates,((ComboItem)targetRepsList.getSelectedItem()).getKey());
	       //layout_names_list_model = new DefaultComboBoxModel<ComboItem>( dest_layout );
	       //layoutList.setModel(layout_names_list_model);
	       //layoutList.setSelectedIndex(0);
	      destLayoutLabel.setText(dest_layout[0].toString());
	     break;
	 case 4:
         ...
         if (e.getSource() == targetRepsList) {
	        ...
	        dest_layout = session.getDatabaseRetriever().getLayoutDestinationsForSourceID(old_plate_layout_name_id,((ComboItem)sampleRepsList.getSelectedItem()).getKey(),((ComboItem)targetRepsList.getSelectedItem()).getKey());
	
	      destLayoutLabel.setText(dest_layout[0].toString());
	      
	     target_layout_names = session.getDatabaseRetriever().getTargetLayoutNamesForProject( session.getProjectID(), ((ComboItem)targetRepsList.getSelectedItem()).getKey());
	...
	int number_of_sample_replicates = session.getDatabaseRetriever().getNumberOfReplicatesForPlateLayout(selected_layout);
