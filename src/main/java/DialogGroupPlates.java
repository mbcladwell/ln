...
public class DialogGroupPlates extends JDialog {
  ...
  public DialogGroupPlates(Session _s, Set<String> _plates, String _format) {
    ...
    //ComboItem[] plateLayouts = session.getDatabaseRetriever().getSourcePlateLayoutNames(Integer.parseInt(format),((ComboItem)typeList.getSelectedItem()).getKey());
   
    //layoutList = new JComboBox<ComboItem>(plateLayouts);
    //layoutList.setSelectedIndex(0);
    label = new JLabel(session.getDatabaseRetriever().getLayoutNameDescrForLayoutID(layout_id));
    c.gridx = 1;
    c.gridy = 7;
    c.gridheight = 1;
    c.gridwidth = 1;
