...
CREATE OR REPLACE FUNCTION new_user(_name character varying, _tags character VARYING, _password CHARACTER VARYING, _group_id INTEGER)
  RETURNS void AS
$BODY$
BEGIN
...
CREATE OR REPLACE FUNCTION new_plate_set(_descr VARCHAR(30),_plate_set_name VARCHAR(30), _num_plates INTEGER, _plate_format_id INTEGER, _plate_type_id INTEGER, _project_id INTEGER, _plate_layout_name_id INTEGER, _lnsession_id INTEGER, _with_samples boolean, _target_layout_name_id INTEGER)
  RETURNS integer AS
$BODY$
DECLARE
   ps_id INTEGER;
...
CREATE OR REPLACE FUNCTION new_target(_project_id INTEGER, _trg_name varchar(30), _descr varchar(250), _accs_id varchar(30))
  RETURNS integer AS
$BODY$
DECLARE
   v_id integer;
...
CREATE OR REPLACE FUNCTION new_target_layout_name(_project_id INTEGER,  _trg_lyt_name varchar(30), _descr varchar(250), _reps INTEGER, q1_id INTEGER, q2_id INTEGER, q3_id INTEGER, q4_id INTEGER)
  RETURNS void AS
$BODY$
DECLARE
   v_id integer;
...
CREATE OR REPLACE FUNCTION bulk_target_upload( _fields text[])
   RETURNS void AS
$func$
DECLARE
   m text[];
...
CREATE OR REPLACE FUNCTION new_hit_list(_name VARCHAR(250), _descr VARCHAR(250), _num_hits INTEGER, _assay_run_id INTEGER, _lnsession_id INTEGER, hit_list integer[])
  RETURNS void AS
$BODY$
DECLARE
 hl_id INTEGER;
...
RETURNS TABLE(assay_run_sys_name VARCHAR,  plate_set_sys_name VARCHAR(32),  plate_sys_name VARCHAR(32), plate_order INT, well_name VARCHAR, type_well VARCHAR,  by_col INTEGER, response REAL, bkgrnd_sub REAL, norm REAL, norm_pos REAL, p_enhance REAL, sample_sys_name VARCHAR(32), accs_id VARCHAR, target_name VARCHAR, target_accs VARCHAR)  AS
$BODY$
DECLARE
...
CREATE TRIGGER delete_neg_value BEFORE INSERT ON assay_result_pre
